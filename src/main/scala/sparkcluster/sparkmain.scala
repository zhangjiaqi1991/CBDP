...
object sparkmain {
    ...
    def main(args: Array[String]) {
        ...
        val location = sc.textFile("hdfs://"+MASTERIP+":9000"+FILE)
            .filter(str => (str.split(",")(STATUS) == "1"))
            .map(s => s.split(","))
...
//            .saveAsTextFile(master+resultfile+ "/location")
     ...
     * @return
     */
    def mainfunction(l: RDD[Tuple2[Int, DenseVector[Double]]], sc: SparkContext):
    RDD[Tuple2[Int, Int]] = {
    ...
    def subclass(
                    ...
                    r: RDD[(Int, Int)],
                    C:Int,
                    sc:SparkContext):Tuple2[RDD[(Int, Int)], RDD[(Int,DenseVector[Double])]]={
        var j,k = (-1)
...
//            .map { s => k += 1; (k, s) })
        ...
        val result = mainfunction(indexlocation, sc)
